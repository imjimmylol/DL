...
class DQNAgent:
    ...
    def run(self, episodes=1000):
        for ep in range(episodes):
            ...
            while not done and step_count < self.max_episode_steps:
                ...
                raw, reward, term, trunc, _ = self.env.step(action)
                next_state = self.preprocessor.step(raw)
                done = term or trunc

                # TD-error for new samples: max-priority so they get sampled at least once
                max_prio = (self.replay_buffer.base.priorities.max()
                            if self.replay_buffer.base.buffer else 1.0)

                # 3) add via the wrapper
    ...
    def train(self):
        # only start learning once we've filled up replay_start_size full n-step transitions
        if len(self.replay_buffer.base.buffer) < self.replay_start_size:
            return 
        ...
        # Sample a mini-batch of n-step transitions + IS weights + indices for priority update
        ...
        # Double-DQN + n-step bootstrap target
        with torch.no_grad():
            # 1) pick actions with online net
            next_q_online  = self.q_net(next_states)
            next_actions   = next_q_online.argmax(dim=1)
            # 2) evaluate with target net
            next_q_target  = self.target_net(next_states) \
                                .gather(1, next_actions.unsqueeze(1)) \
                                .squeeze(1)
            # 3) assemble n-step target: rewards already = R⁽ⁿ⁾
        ...
        # 6) update priorities in the underlying buffer
